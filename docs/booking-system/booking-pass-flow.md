...
The new-Pass/Drop-in cards and Review summary are implemented for desktop and mobile. Checkout is explicitly unavailable in the public flow: selection/review create no Hold, Cart, payment or booking. Internally, NEW_PASS and Session-derived DROP_IN now run through live Shopify checks, a 15-minute Hold, one durable Cart creation claim, verified Cart read-back and a server-only Cart secret. Development-store product readiness passes, but the public route remains behind checkoutAvailable=false until paid-webhook confirmation and recovery exist. Entitlement ledger primitives, eligible-owned-Pass ordering and conservative Intro history are implemented internally. Owned Pass cards, customer identity summary, atomic confirmation, front-end Checkout hand-off and payment recovery remain pending. This is a partial implementation of the full target contract below.
...
Development-store product readiness passed on 2026-09-12 at 05:34:57–58 UTC: after explicit user authorization, the product-read scope and existing TOML definitions were applied, both test products received CAS-protected missing ownership values and were published only to the verified Online Store publication. Both reports returned ready=true; store password protection remains enabled. Admin/Review use the official SDK's offline authenticated Storefront client, verify the shop and exact unauthenticated_read_product_listings scope, and fail closed when absent. An explicit maintenance helper can reconcile a stale local scope cache from verified Shopify grants without modifying tokens.

Admin verifies entitlement_kind (DROP_IN/PACK). Published test products returned onlineStoreUrl=null despite confirmed publication, so readiness now requires a valid non-future Online Store publishedAt timestamp plus independent live AU Storefront checks; a URL alone is not proof of publication. This remains a read-only observation, not durable purchase authorization. Real-account Continue with Shop / Review, public Hold → Cart/Checkout, paid webhooks and Booking confirmation remain unverified or unimplemented; all three transaction flags stay false. See [Commerce readiness](commerce-readiness.md) for exact targets, evidence and safe maintenance commands.
