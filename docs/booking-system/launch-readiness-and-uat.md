...
客户个人中心（Upcoming/History/My Passes）、取消/改期、Coach 名册/签到/出席、Admin 预约管理与基础 Reports 已完成本地实现。11 条迁移已应用。经营者以后补银行、身份和邮箱配置。下表中的早期缺口按本段更新；真实 UAT 的 30 组仍全部待实际账号签收，自动化通过不替代真实签收。详细边界见 [本轮说明](customer-account-and-lifecycle.md) 与 [交接](handoff-2026-09-13.md)。

更新：2026-09-13。本轮已重新执行自动化与浏览器测试；未配置真实支付、邮箱或真实客户账号页面。

## 如何理解剩余工作

当前 300 项自动化测试通过，Coach 和 Customer 各 2 个尺寸的浏览器验证通过；另保留上一轮 Home/Programs 16 组场景证据。它们主要覆盖当前已实现逻辑与本地 fixture，不等于整站或真实 Shopify 支付/邮件验收。Backlog 的旧勾选项存在“代码已实现但完整交付未签收”，不能简单统计未勾选项或用测试数量计算完成百分比。
...
| Customer 与取消改期 | Home/Programs 选课/确认，Account 预约历史/Pass，取消/改期 | Overview 聚合、客户真实资料、日历和留言、真实账号接通；My Passes/History、取消/改期已实现 |
| Coach 日常操作 | 安全登录、7/30 天及自定义统计、名册/签到/出席/No-show 与审计 | 真邮箱邀请、真实姓名解析、Today dashboard、老师留言、Availability |
...
| Appointment 与扩展排期 | 部分通用模型/冲突基础 | 私教可用时段、周期 availability/time off、资源冲突、审批和整套流程；周期排课生成 |
...
| UAT-06 | 身份 · 账户切换与隔离 | 两客户/两 Coach 互不可读写他人的 Pass、Booking、排期或内部留言。 | 待整链路签收 |
...
| UAT-21 | Coach · 名册和签到 | Roster、Attended、No-show、客户可见/内部留言；课次结算与隔离一致。 | 待整链路签收 |
...
| UAT-28 | Appointment · 私教完整流程 | 可预约时段、Coach availability/time off、资源/buffer 冲突、审批、取消/改期、通知。 | 待整链路签收 |
