...
try {
  ...
  for (const width of [390,1440]) {
    const f = await paidFixture(); await db.bookingAttempt.update({where:{id:f.attempt.id},data:{customerComment:"Core strength <script>unsafe()</script>"}}); await db.customerProfile.update({where:{id:f.customer.id},data:{preferredName:"Queenie",signature:"Stronger every class",trainingGoals:"Build core strength and confidence"}}); await processPaidBookingEvent((await queuePaid(f)).id); const booked = await db.booking.findFirstOrThrow({where:{shopId:f.shop.id}});
    ...
    await page.getByRole("navigation", { name: "Coach portal" }).waitFor();
    await page.getByRole("link", { name: "Today", exact: true }).waitFor();
    await page.getByRole("link", { name: "My schedule", exact: true }).waitFor();
    assert.equal(await page.getByRole("navigation", { name: "Coach portal" }).getByRole("link").count(), 2);
    await page.getByRole("heading", { name: "Booking notifications" }).waitFor();
    ...
    await page.getByLabel("Show").selectOption("month");
    ...
    await page.getByRole("region", {name:"Training profile for Queenie"}).waitFor();
    await page.getByText("Build core strength and confidence",{exact:true}).waitFor();
    ...
    await page.reload();
    assert.deepEqual(await page.getByLabel("Booking action").locator("option").allTextContents(),["Mark no-show"]);
    await page.getByLabel("Reason").fill("Customer did not attend");
    ...
    await page.getByLabel("Show").selectOption("custom");
    await page.getByLabel("From (custom dates)").fill("2020-01-01");
    await page.getByLabel("To (custom dates)").fill("2020-01-02");
    await page.getByRole("button", { name: "Apply dates" }).click();
    await page.getByText("No sessions in this date range", { exact: false }).waitFor();
