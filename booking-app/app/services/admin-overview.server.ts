  ...
  return db.$transaction(
    async (tx) => {
      ...
      const [sessions, attention, expiringPasses] = await Promise.all([
        ...
        >`SELECT e.id AS "entitlementId", e."customerId", COALESCE(NULLIF(c."preferredName", ''), 'Customer ' || RIGHT(e."customerId"::text, 8)) AS "customerName", COALESCE(p.name, s.name, 'Class credit') AS "passName", e."expiresAt", e."grantedUnits"::int AS granted, SUM(l."availableDelta" + l."reservedDelta")::int AS remaining FROM "Entitlement" e JOIN "CustomerProfile" c ON c.id=e."customerId" AND c."shopId"=e."shopId" LEFT JOIN "PassPlan" p ON p.id=e."passPlanId" AND p."shopId"=e."shopId" LEFT JOIN "Service" s ON s.id=e."serviceId" AND s."shopId"=e."shopId" JOIN "EntitlementLedgerEntry" l ON l."entitlementId"=e.id AND l."shopId"=e."shopId" WHERE e."shopId"=${shop.id}::uuid AND e.status='ACTIVE' AND e."startsAt"<=${now} AND e."expiresAt">${now} AND e."expiresAt"<=${expiryEnd.toJSDate()} GROUP BY e.id,e."customerId",c."preferredName",p.name,s.name,e."expiresAt",e."grantedUnits" HAVING SUM(l."availableDelta" + l."reservedDelta")>0 ORDER BY e."expiresAt" ASC, e.id ASC`,
      ]);
      return {
